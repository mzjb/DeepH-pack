...
from deeph import get_graph, DeepHKernal, collate_fn
...
def main():
    ...
    if old_version is False:
        checkpoint = kernal.build_model(args.trained_model_dir, old_version)
    else:
        ...
        kernal.build_model(args.trained_model_dir, old_version)
    ...
    with torch.no_grad():
        ...
        if args.save_csv:
            ...
            atomic_numbers = torch.squeeze(kernal.index_to_Z[batch.x[batch.edge_index.T]]).tolist()
            ...
            with open(os.path.join(kernal.config.get('basic', 'save_dir'), 'error_distance.csv'), 'w', newline='') as f:
                ...
                writer.writerow(['index', 'atom_id', 'atomic_number', 'dist', 'atom1_x', 'atom1_y', 'atom1_z',
                                ...
                                + ['target'] * kernal.out_fea_len + ['pred'] * kernal.out_fea_len + [
                                    'mask'] * kernal.out_fea_len)
